...
静态 mermaid 适合写进文档；**运行时**请装 [DevTools 扩展](../advanced/devtools)：

1. 加载仓库中的 `devtools/dist`（Chrome / Edge 开发者模式）
2. 打开你的页面或本站 Playground，按 <kbd>F12</kbd>
3. 切换到 **AFSM** 标签，即可看到实例树、状态图与时间线

配合 [DevTools 扩展](../advanced/devtools)，可以在浏览器开发者工具里实时看到当前状态机图，并高亮当前 / 过渡态。
...
- [DevTools 扩展](../advanced/devtools) — 安装、面板说明与排错
